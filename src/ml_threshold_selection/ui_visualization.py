...
# Fig 3 of the article. Shading them by below-threshold-class probability repeated the
...
def export_publication_fig3(app):
    """
    Export the legacy GUI probability plot.

    This is not the corrected manuscript Fig 3; that figure is generated by
    ``generate_main_figures.py`` from released inputs.
    """
    import os
    from datetime import datetime
    import numpy as np
    ...
    # by below-threshold-class probability, which the vertical axis already carries, so the
    ...
    ax1.set_ylabel('Below-threshold class probability', fontweight='bold', color='black')
    ax1.tick_params(axis='y', labelcolor='black')
    ax1.set_ylim(-0.05, 1.05)
    
    # Setup ax2 (twinx) for the mean curve
    ax2 = ax1.twinx()
    ax2.set_ylabel('Retained mean class probability', fontweight='bold', color='darkred')
    ...
    for t in thresholds:
        ...
        retained_mean_probabilities.append(float(np.mean(app.probabilities[retained])) if np.sum(retained) > 0 else 0.0)
    
    ax2.plot(thresholds, retained_mean_probabilities, color='darkred', linewidth=2.5, label='Retained mean class probability')
    ...
    # Threshold Lines
    lines = []
    lines.append(Line2D([0], [0], color='darkred', lw=1.5, label='Retained mean class probability'))
...
def show_training_visualization(app):
    ...
    below_threshold_proba = app.training_results['train_proba'][app.training_results['y'] == 1]
    axes[1, 0].hist(normal_proba, bins=30, alpha=0.7, label='Retained-class objects', color='blue')
    axes[1, 0].hist(below_threshold_proba, bins=30, alpha=0.7, label='Below-threshold-class objects', color='red')
...
def show_prediction_visualization(app):
    ...
    axes[0, 1].set_ylabel('Below-threshold class probability', fontweight='bold', color='black', fontsize=11)
    # Set explicit tick label sizes
    axes[0, 1].tick_params(axis='both', labelsize=9)
    axes[0, 1].tick_params(axis='y', labelcolor='black')
    
    axes[0, 1].set_ylim(-0.05, 1.05)
    axes[0, 1].set_title('Pseudo-label probability vs minimum volume', fontweight='bold', fontsize=12)
    
    ax01_twin = axes[0, 1].twinx()
    ax01_twin.set_ylabel('Retained mean class probability', fontweight='bold', color='darkred')
    ...
    retained_mean_probabilities_gui = []
    for t in thresholds_gui:
        retained = v_min_mm3 >= t
        retained_mean_probabilities_gui.append(float(np.mean(app.probabilities[retained])) if np.sum(retained) > 0 else 0.0)
    
    ax01_twin.plot(thresholds_gui, retained_mean_probabilities_gui, color='darkred', linewidth=2.5, label='Retained mean class probability')
    
    from matplotlib.lines import Line2D
    lines_01 = [Line2D([0], [0], color='darkred', lw=1.5, label='Retained mean class probability')]
    ...
    retention_rates, retained_mean_probabilities = [], []
    for t in thresholds:
    ...
    line2 = ax2.plot(np.log10(thresholds), retained_mean_probabilities, 'r-', label='Retained mean class probability')
