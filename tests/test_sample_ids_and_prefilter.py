...
from ml_threshold_selection.data_io import derive_test_sample_id


def test_sample_ids_preserve_meaningful_underscore_segments():
    raw_cases = {
    ...
    for filename, expected in raw_cases.items():
    ...
    for filename, expected in processed_cases.items():
        assert extract_sample_id_from_processed_xlsx(filename) == expected
        assert derive_test_sample_id(filename) == expected


def test_prefilter_excludes_invalid_eigenvalues_without_shape_substitution():
    df = pd.DataFrame({
...
def test_prefilter_requires_all_eigenvalue_columns():
